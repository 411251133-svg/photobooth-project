...
(function () {
  ...
  function renderGallery(items = []) {
    ...
    if (!items.length) {
      galleryGrid.innerHTML =
        '<div style="color:var(--text-gray);padding:1rem">Galeri kosong</div>';
      return;
    }
    ...
    items.forEach((item) => {
      ...
      info.innerHTML = `<div style="font-size:13px;color:var(--text-gray)">${
        item.filename
      }</div><div style="font-size:12px;color:var(--text-gray)">${new Date(
        item.createdAt
      ).toLocaleString()}</div>`;
